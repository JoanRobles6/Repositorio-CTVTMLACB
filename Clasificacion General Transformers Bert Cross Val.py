...
def ejecutar_experimento_bert_cv(x, y):
    ...
    for nombre, modelo in modelos.items():
        ...
        plt.title(f'Matriz de Confusión Final - {nombre} (BERT)')
        plt.savefig(f'graficos_bert_cv/matriz_confusion_{nombre}.png', dpi=300, bbox_inches='tight')
        plt.show()
        ...
        if hasattr(modelo, "predict_proba"):
            ...
            plt.title(f'Curva ROC Final - {nombre} (BERT)')
            plt.legend()
            plt.grid(True)
            ...
            plt.legend()
            plt.grid(True)
            plt.savefig(f'graficos_bert_cv/distribucion_prob_{nombre}.png', dpi=300, bbox_inches='tight')
            plt.show()

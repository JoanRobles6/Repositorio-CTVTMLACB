...
def ejecutar_experimento_d2v(x, y):
    ...
    d2v_model = Doc2Vec(vector_size=100, window=5, min_count=1, workers=10, epochs=20)
    ...
    for nombre, modelo in modelos.items():
        ...
        plt.title(f'Matriz de Confusión Final - {nombre} (Doc2Vec)')
        plt.savefig(f'graficos_d2v/matriz_confusion_{nombre}.png', dpi=300, bbox_inches='tight')
        plt.show()
        ...
        if hasattr(modelo, "predict_proba"):
            ...
            plt.title(f'Curva ROC Final - {nombre} (Doc2Vec)')
            plt.legend()
            plt.grid(True)
            plt.savefig(f'graficos_d2v/curva_roc_{nombre}.png', dpi=300, bbox_inches='tight')
            plt.show()
            ...
            plt.title(f"Densidad de Probabilidades - {nombre} (Doc2Vec)")
            plt.legend()
            plt.grid(True)
            plt.savefig(f'graficos_d2v/distribucion_prob_{nombre}.png', dpi=300, bbox_inches='tight')
            plt.show()
